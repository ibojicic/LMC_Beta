...
## flag g: 20cm limited to 5, 40cm limited to 6 
data.radio.alphas.twolowest.limits_g <- data.spindex %>% filter(flag == 'g', !is.na(lin_slope)) %>% select(-flag)


# filter fluxes and spindex for alphas number density
data.radio.numberdensity.limits_g <- merge(data.radio.fluxes, data.radio.alphas.twolowest.limits_g, by ='idPNMain')
...
data3 <- data.radio.numberdensity.limits_g
